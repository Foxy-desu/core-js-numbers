...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModuleProduct =
    Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(dotProduct / vectorModuleProduct);
}
...
function roundToPowerOfTen(num, pow) {
  const powerNumber = Number(`1${'0'.repeat(pow)}`);
  const decimall = num / powerNumber;
  const result = Math.round(decimall) * powerNumber;
  return result;
}
...
function getSine(num) {
  return Math.sin(num, 10).toFixed();
}
...
function getSumOfNumbers(...arg) {
  return arg.reduce((acc, current) => acc + current).toFixed(1);
}
